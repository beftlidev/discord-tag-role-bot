  ...
  run: async (client, interaction) => {
    ...
    if (await BlackList.fetch(`blacklistedUsers.${interaction.user.id}`)) {
      ...
      return interaction.reply({
        ...
        components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel("Support Server").setURL("https://discord.gg/TCWbk7zWY5"))],
        ephemeral: true
      })
    }
    ...
    if (command.commandAuthType == "User") {
      ...
      if (!client.channels.cache.get(interaction.channel.id).guild.members.me.permissionsIn(interaction.channel.id).has([PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.AttachFiles])) {
        ...
        return interaction.reply({
          embeds: [
            await errorEmbed({
              description: `Hey, one or both of \`Send Messages, Attach Files\` authorisations are not available in this channel. Therefore, commands are restricted on this channel. Please contact an authorised person.`
            })
          ],
          ephemeral: true
        })
    ...
    try {
      ...
      await sendErrorReport(client, interaction, "slash command", interaction.commandName, e)
    }
